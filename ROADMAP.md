...
The active product direction is now Phase 8E: a dashboard-ready fixed floor roster, persistent workstation ownership, per-character stamina and a small deterministic behavior loop. The 8E.W four-way Work/WorkSeat direction slice and the PC workstation frame-channel integration are complete and author-approved; current authored world slots still contain no NE record, but the derived NE runtime path is ready for future use. The talk route/session bridge is engineering-complete and core-verified, and the final local web review host is now rebuilt and browser-checked with deterministic Talk/Effects demos. The follow-up frame pass now swaps decoded raster frames atomically (no cross-fade), carries completed normal-work loops for the PC cell channel, and waits through cooldown-only behavior gaps; author visual/behavior/tuning acceptance remains open. Multi-actor workstation queues and crowd slot contention are no longer on the active roadmap; they remain optional future work only if a later product need proves them necessary.
...
The talk route/session boundary and final local review host are implemented and verified. The remaining gate is explicit author visual/behavior acceptance plus stamina/content approval.

1. Author-check the browser Talk demo: portal spawn/seat, outbound route, arrival bubbles with the 500ms reply gap, fade, optional shared sad/happy result and the initiator's full return to its owned WorkSeat.
2. Observe and approve/revise initial stamina drain/recovery tuning (`sad -1`, `happy +2`, work drain/recovery timing); technical values remain provisional until gameplay approval.
3. Review Effects and home/return demos, including assignment retention and explicit ready-gated `request_return`, then review Thai dialogue shaping/content.
4. Keep `CHARACTER/DIALOGUE/dialogue.csv` as the editable source and re-enable fit-gated rows only after renderer fit/reload checks.
5. Replace temporary TV bubble/font development inputs, create a fresh clean release archive with fresh-extraction verification, and only then close Phase 8E. Do not close from generated reports alone.
