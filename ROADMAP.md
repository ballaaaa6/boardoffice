...
9. **8E.6 — automated and visual verification:** implementation tests and required audits are run as one final batch after this slice; the local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` host starts a self-running deterministic floor scenario from portal spawn, keeps the authored spawn → walk → WorkSeat order, exposes the available talk/speech/VFX/HumanBall channels and critical home/return without driving every tick manually, and uses compact live frames while keeping full save/load/replay state. Browser/performance audit shows the review host is not yet smooth or complete enough for acceptance (server-rendered full-frame updates, queued talk, floor02-only/English-seed0 UI and hidden route samples); review-host architecture, coverage and author acceptance remain open.
...
2. Embed `RuntimePresentationHostAdapter.tick()` into the external app's live participant loop without changing conversation geometry, `60/360/240/240ms` timing or the assignment contract. The reducer-to-speech/VFX/HumanBall/movement bridge, numeric emotion bridge, stateful loop, host callback adapter and local review host are implemented; external app wiring is the remaining integration seam.
3. Validate the implemented home/return behavior in the host renderer and through persisted replay: `going_home` → portal exit/despawn → `home_recovery` → ready-gated portal entry → `returning_to_work` → the same owned WorkSeat, with full stamina restore only at the declared home-recovery boundary. The local web host exposes this route and replay controls.
4. Keep assignment/unassignment as explicit dashboard commands and keep durable storage outside Central Core; the JSON snapshot codec and byte-equivalent explicit-step replay seam are now implemented and covered by focused tests.
