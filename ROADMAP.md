...
9. **8E.6 — automated and visual verification:** implementation tests and required audits are run as one final batch after this slice; the local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` host starts a self-running deterministic floor scenario so the author can watch stamina, loop boundary, work/recovery/talk/wander activity, home/return and replay without driving every tick manually. Only external app embedding and explicit author acceptance remain.
