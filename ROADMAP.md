...
9. **8E.6 — automated and visual verification:** the deterministic core talk gate is passed: request → route → arrival → bubbles → optional sad/happy → return, with self-talk fallbacks, cancellation/timeout and no stuck `talking` covered by focused traces plus the full regression/audit batch. The final local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` review host now starts actors at the portal, uses compact WebP + double-buffered crossfade and generation-safe requestAnimationFrame pacing, exposes route/talk/VFX/HumanBall streams, accepts EN/TH + seed settings, and provides deterministic Talk/Effects demos that auto-pause at their authored end boundary. The trusted fast path is review-host-only; default Central/actor/speech APIs retain validation/copy isolation. Browser QA at `http://127.0.0.1:8765/` and the focused review-server/renderer tests pass. Visual/behavior/Thai-content author review and stamina tuning remain open.
