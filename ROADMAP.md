...
9. **8E.6 — automated and visual verification:** implementation tests and required audits are run as one final batch after this slice; the local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` host starts a self-running deterministic floor scenario from portal spawn, keeps the authored spawn → walk → WorkSeat order, exposes visible talk/speech/VFX/HumanBall activity and critical home/return without driving every tick manually, and uses compact live frames while keeping full save/load/replay state. Only external app embedding and explicit author acceptance remain.
