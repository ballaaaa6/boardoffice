...
The active product direction is now Phase 8E: a dashboard-ready fixed floor roster, persistent workstation ownership, per-character stamina and a small deterministic behavior loop. The 8E.W four-way Work/WorkSeat direction slice and the PC workstation frame-channel integration are complete and author-approved; current authored world slots still contain no NE record, but the derived NE runtime path is ready for future use. The talk route/session bridge is engineering-complete and core-verified, and the final local web review host is now rebuilt and browser-checked with deterministic Talk/Effects demos. The follow-up frame pacing/decode/retry pass is implemented and browser-verified; author visual/behavior/tuning acceptance remains open. Multi-actor workstation queues and crowd slot contention are no longer on the active roadmap; they remain optional future work only if a later product need proves them necessary.
...
9. **8E.6 — automated and visual verification:** the deterministic core talk gate is passed: request → route → arrival → bubbles → optional sad/happy → return, with self-talk fallbacks, cancellation/timeout and no stuck `talking` covered by focused traces plus the full regression/audit batch. The final local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` review host starts actors at the portal, uses compact WebP + double-buffered decode-safe swaps, advances on fixed 60ms simulation slices without adding network latency to every interval, exposes route/talk/VFX/HumanBall streams, accepts EN/TH + seed settings, and provides deterministic Talk/Effects demos with opt-in auto-pause at their authored end boundary. Transient fetch failures retry with timeout/backoff while the prior image remains visible. The trusted fast path is review-host-only; default Central/actor/speech APIs retain validation/copy isolation. Browser QA at `http://127.0.0.1:8765/` and the focused review-server/renderer/web tests pass. Visual/behavior/Thai-content author review and stamina tuning remain open.
