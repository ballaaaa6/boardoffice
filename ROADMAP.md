...
9. **8E.6 — automated and visual verification:** implementation tests and required audits are run as one final batch after this slice; first the talk behavior must pass a deterministic core-only trace gate (request → route → arrival → bubbles → optional sad/happy → return, including queue fairness and no stuck `talking`). Only then will the local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` host be rebuilt once for final inspection. Browser/performance audit shows the current review host is not yet smooth or complete enough for acceptance (server-rendered full-frame updates, queued talk, floor02-only/English-seed0 UI and hidden route samples); review-host architecture, coverage and author acceptance remain open.
