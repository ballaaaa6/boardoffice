...
3. **8E.W — Work pose, WorkSeat direction and PC frame-channel completeness:** implementation complete; the 360/240/240 timing-policy visual re-approval is pending — canonical Work has fixed-head/alternating-body turns with exact `turn_side_<direction>` names, native M42–M45 composite frame rules, derived SW/NE final-frame mirrors and a callable four-way NE action series from NW. The runtime WorkSeat bridge now derives a complete NE workstation composite from NW with one mirror relation for character, chair, desk, PC, optional foreground, offsets, VFX and HumanBall channels while preserving source draw layers. PC cells 1–5 are recovered for all 25 active PC families; NW/NE advances one cell per complete work-action loop while SE/SW remain on cell0. The canonical presentation timing is character 360 ms / VFX 240 ms / HumanBall 240 ms on the shared 60 ms tick; current authored world slots remain 100 SE / 96 NW / 23 SW / 0 NE and floor geometry/static floor hashes remain unchanged.
