...
### Phase 8E — dashboard-ready roster, stamina and behavior loop — conversation one-loop presentation, deterministic actor reducer, speech scheduler and host-loop adapter implemented; app embedding and acceptance pending
...
The read-only runtime presentation seam now has a concrete consumer in `RUNTIME/runtime_presentation_renderer.py` and a stateful `RuntimePresentationLoop` host adapter, plus a three-mode/home-return QA harness that drives the adapter. `RUNTIME/runtime_presentation_host.py` now adds the explicit `RuntimePresentationHostAdapter` callback boundary for an external app: one host tick produces one frame and ordered events, while `render_current()` redraws without advancing time. Remaining work is wiring that boundary into the external app's participant loop, visual/behavior author acceptance, numeric sad/happy stamina tuning and the dashboard-owned persistence/replay boundary. No windowing/UI host exists in this repository. No new floor geometry or navigation cells are required for the conversation slice; the PC cells above are the explicitly approved source-sheet recovery for workstation presentation.
...
The independent speech channel is now contract-first and JSON-safe in `RUNTIME/speech_scheduler_core.py`. Every assigned actor receives deterministic lifecycle, solo and pair timers; the solo pool is `encouragement`, `uncertainty`, `surprise`, `work_progress` and `idle_flavor`; pair turns are `conversation_open` then `conversation_reply`; and `greeting`, `work_start`, `fatigue` and depth-gated `leaving` are one-shot lifecycle lines. All delays quantize to the shared 60ms tick: greeting 2–3s after spawn, solo 30–60s, pair 45–75s, 4,000ms visible plus 300ms fade. A floor speech lane prevents a new session while the current bubble/fade window is owned; the two bubbles inside one pair are the intentional same-session overlap. The scheduler emits the session/movement boundary first and a separate bubble-start event at arrival, so pose/movement and BB timing remain independent.
 ...
 Pair requests are filtered to same-floor actors that are present, working and unlocked. A seeded draw chooses uniformly among currently available `ceo_front`, `seated_host` and `standing_pair` modes, with deterministic partner candidates and plan-validity retries. CEO conversations keep the CEO in `work/normal_work`; an employee visitor is `idle`; a seated employee host uses its named `turn_side_<direction>` Work pose; and standing pairs use inverse `idle` facings at a runtime-resolved map spot. A standing pair rolls one shared deterministic d6 only after the shared bubble/fade completes: even is `happy`, odd is `sad`; both hold the result for 1,200ms, then the return hook is emitted. `CentralGameCore.resolve_runtime_snapshot()` and `advance_runtime_snapshot()` compose the actor, speech and conversation channels without sharing mutable timers, stamina or workstation ownership. `CentralGameCore.resolve_runtime_presentation()` materializes the read-only `gds.runtime_presentation_snapshot.v1` actor render records, active/returning conversation tracks, bubble opacity/paint order, VFX/HumanBall channels and 360/240/240 frame clocks. `RuntimePresentationRenderer` consumes those records with independent per-assignment character/VFX/HumanBall/PC indices, walking-depth masking, direct head anchors and route-preserving lifecycle bubbles; `RuntimePresentationLoop` advances Central and renders one floor per host tick, preserving the last good snapshot on failure. Its QA harness covers all three modes plus home/return. The render-depth helper `actor_draws_over_reception()` supplies the explicit leaving trigger from the authored reception front envelope. Focused scheduler/presentation/host-loop tests, full regression and the required navigation/WorkSeat/spatial/integrity/F2/conversation audits pass; app embedding, visual/behavior author acceptance, Thai/content review and numeric emotion tuning remain open.
...
The metadata contract now records the initial tuning policy: stamina max 100, critical threshold <=10, target work cycle 120–300 seconds, per-employee work drain 600–850 milli-stamina/second, talk recovery 5–9, background-effect recovery 1–3, popup recovery 1–2, wander recovery 1–4, and home delay 8–20 seconds. Event intervals and activity durations are also stored as seeded ranges. The actor reducer and speech scheduler consume these values deterministically; conversation participant selection, renderer event channels and home/return composition are implemented through the runtime snapshot seam, while final numeric emotion tuning remains an open author decision.
...
4. **8E.1 — roster and ownership resolver:** in progress — static initial ownership is wired and validated; the actor snapshot contract freezes assignment identity/immutability and active-map scope, while mutable snapshot assignment, explicit vacancy commands and all-floor mutation validation remain.
5. **8E.2 — actor snapshot and stamina reducer:** runtime slice implemented — stable per-character profiles, deterministic state-in/state-out advancement, exact milli-stamina drain/remainder carry, downward threshold events, weighted behavior starts, renderer-channel bindings and clamped recovery are implemented. Home routing now joins this reducer through 8E.4.
6. **8E.3 — talk, wander and popup behaviors:** conversation movement and the initial talk timing/presentation are implemented and visually author-approved, including direct head-anchor overlay paint order — one deterministic pair exchange, `conversation_open` → `conversation_reply` content selection, 500ms speaker gap, 4,000ms global bubble window, 300ms fade, seated turn-side hold and assignment-safe return. The independent speech scheduler adds lifecycle (`greeting`, `work_start`, `fatigue`, `leaving`), five solo pools, 30–60s/45–75s seeded intervals, no-new-bubble lane gating, arrival-boundary bubble events, three-mode valid selection and standing-pair shared sad/happy outcome before return. Weighted actor events now emit conversation/VFX/HumanBall/movement bindings and Central forwards effective talk/depth boundaries; `resolve_runtime_presentation()` plus `RuntimePresentationRenderer` materialize/consume the cross-channel render snapshot while preserving separate pose/stamina/assignment clocks, independent per-assignment frame indices, walking depth and lifecycle route poses. Runtime QA covers all three pair modes and home/return. Numeric sad/happy stamina tuning, host/live renderer integration and explicit author acceptance remain pending. Thai shaping/content quality remains a separate review gate.
...
8. **8E.5 — dashboard-facing Central facade:** JSON-safe actor, speech and composed runtime snapshot/validation/advance/behavior-choice methods plus actor-to-presentation timestamp bridges are exposed; explicit assignment mutation, live persistence orchestration and deterministic replay integration remain pending, with no persistence backend in Central Core.
9. **8E.6 — automated and visual verification:** the reducer/home/presentation slice has focused contract/runtime coverage and the latest full regression recorded in the handoff, with passing Room Navigation, Navigation Occupancy, WorkSeat, WorkSeat Lifecycle, Phase 6 Spatial, Central integrity, F2 gameplay-metadata family and conversation audits. The `RuntimePresentationLoop`-driven QA harness produced PASS contact sheets for `ceo_front`, `seated_host`, `standing_pair` and home/return, including the lifecycle-route regression fix. App embedding, Thai/content review, numeric emotion tuning and explicit author acceptance of the new runtime integration remain pending.
...
The in-repository host boundary is now implemented and verified. The next integration call belongs to the external app/window host, which should invoke `RuntimePresentationHostAdapter.tick()` exactly once per participant frame and present the returned frame/events.
...
2. Embed `RuntimePresentationHostAdapter.tick()` into the external app's live participant loop without changing conversation geometry, `60/360/240/240ms` timing or the assignment contract. The reducer-to-speech/VFX/HumanBall/movement bridge, standing-pair emotion pose binding, stateful loop, host callback adapter and review QA harness are implemented; external app wiring, numeric sad/happy stamina tuning and explicit author acceptance are the next seam.
3. Validate the implemented home/return behavior in the host renderer and through persisted replay: `going_home` → portal exit/despawn → `home_recovery` → ready-gated portal entry → `returning_to_work` → the same owned WorkSeat, with full stamina restore only at the declared home-recovery boundary. Review-only contact-sheet evidence already covers this route.
4. Finish the dashboard-facing replay seam: explicit commands/events, snapshot persistence ownership, assignment/unassignment validation and byte-equivalent replay tests; keep database/network/UI out of Central Core.
5. Run the required runtime/navigation/WorkSeat/spatial/integrity audits, representative visual QA and explicit author acceptance before any release packaging or Phase 8E closeout.
