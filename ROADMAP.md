...
9. **8E.6 — automated and visual verification:** implementation tests and required audits are run as one final batch after this slice; the local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` host is the author-facing visual check for stamina, loop boundary, home/return and replay. Only external app embedding and explicit author acceptance remain.
