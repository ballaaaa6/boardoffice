...
The active product direction is now Phase 8E: a dashboard-ready fixed floor roster, persistent workstation ownership, per-character stamina and a small deterministic behavior loop. The 8E.W four-way Work/WorkSeat direction slice and the PC workstation frame-channel integration are complete and author-approved; current authored world slots still contain no NE record, but the derived NE runtime path is ready for future use. The talk route/session bridge is engineering-complete and core-verified. The Central frame-normalization/distance propagation pass and talk-plan fast path are now implemented and regression-tested. The API v2/local web review host exposes all 25 floors and all runtime channels with floor/actor/partner controls, deterministic Talk mode selection, Effects/Wander/Critical demos and per-frame telemetry. Author visual/behavior/tuning acceptance remains open. Multi-actor workstation queues and crowd slot contention are no longer on the active roadmap; they remain optional future work only if a later product need proves them necessary.
...
9. **8E.6 — automated and visual verification:** the deterministic core talk gate is passed: request → route → arrival → bubbles → optional sad/happy → return, with self-talk fallbacks, cancellation/timeout and no stuck `talking` covered by focused traces plus the full regression/audit batch. The final local `TOOLS/runtime_review_server.py` + `WEB/runtime_review.html` review host starts actors at the portal, uses compact WebP + double-buffered decode-safe atomic swaps (the prior frame stays opaque until decode), advances on fixed 60ms simulation slices without adding network latency to every interval, exposes route/talk/VFX/HumanBall/stamina/portal/PC streams, accepts EN/TH + seed settings, switches across all 25 floors and provides deterministic Talk/Effects/Wander/Critical demos with opt-in auto-pause at their authored end boundary. Transient fetch failures retry with timeout/backoff while the prior image remains visible; cooldown-only actor gaps defer to the earliest eligible event instead of failing a live tick. The trusted fast path is review-host-only; default Central/actor/speech APIs retain validation/copy isolation. Browser QA at `http://127.0.0.1:8765/`, the 300,060ms live soak and the focused review-server/renderer/web tests pass; the full regression is 314 passed. Visual/behavior/Thai-content author review and stamina tuning remain open.
...
The two engineering blockers from the first browser review are fixed and covered by the 314-test regression/audit bundle. The author gate is now the next boundary; the running host is ready for one end-to-end visual pass rather than patch-by-patch checks.

1. Author browser pass on `http://127.0.0.1:8765/`: verify floor switching across `floor00`, `floor01`, `floor02` and one F2-family floor; observe spawn → walk-to-seat with changing movement frames and PC cells.
2. Verify Talk modes in the host: seeded auto, `self_talk`, `ceo_front`, `seated_host`, `standing_pair`; confirm outbound/arrival/bubbles/reply gap/fade/emotion hold/return and no repeated or stuck speech.
3. Observe and approve/revise initial stamina tuning (`sad -1`, `happy +2`, work drain/recovery timing) and critical finish-current-`normal_work` → home route → ready-gated return; values remain provisional until gameplay approval.
4. Review Effects and Wander demos, VFX/HumanBall timing, route telemetry and assignment retention; review Thai dialogue shaping/content in the same host.
5. Keep `CHARACTER/DIALOGUE/dialogue.csv` as the editable source and re-enable fit-gated rows only after renderer fit/reload checks.
6. Replace temporary TV bubble/font development inputs, create a fresh clean release archive with fresh-extraction verification, and only then close Phase 8E. Do not close from generated reports alone.
